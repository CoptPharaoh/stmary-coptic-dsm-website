...
(function(){
  var links = Array.prototype.slice.call(document.querySelectorAll('.st-side-nav a[href^="#"]'));
  ...
  links.forEach(function(a){ a.addEventListener('click', function(){ setTimeout(setActive, 300); }); });
})();



(function(){
  const links=[...document.querySelectorAll('.st-side-nav a')];
  const sections=links.map(a=>document.querySelector(a.getAttribute('href'))).filter(Boolean);
  const setActive=()=>{
    let current=sections[0];
    for(const sec of sections){ if(sec.getBoundingClientRect().top < 170) current=sec; }
